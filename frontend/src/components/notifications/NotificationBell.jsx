...
const typeConfigBase = {
  exam_schedule: { icon: FiCalendar, color: 'text-blue-500', bg: 'bg-blue-100 dark:bg-blue-900/30', labelKey: 'notifTypeExamSchedule' },
  hall_ticket: { icon: FiFileText, color: 'text-purple-500', bg: 'bg-purple-100 dark:bg-purple-900/30', labelKey: 'notifTypeHallTicket' },
  result: { icon: FiAward, color: 'text-green-500', bg: 'bg-green-100 dark:bg-green-900/30', labelKey: 'notifTypeResult' },
  assignment: { icon: FiFileText, color: 'text-orange-500', bg: 'bg-orange-100 dark:bg-orange-900/30', labelKey: 'notifTypeAssignment' },
  fee_reminder: { icon: FiDollarSign, color: 'text-red-500', bg: 'bg-red-100 dark:bg-red-900/30', labelKey: 'notifTypeFeeReminder' },
  placement: { icon: FiBriefcase, color: 'text-cyan-500', bg: 'bg-cyan-100 dark:bg-cyan-900/30', labelKey: 'notifTypePlacement' },
  announcement: { icon: FiInfo, color: 'text-indigo-500', bg: 'bg-indigo-100 dark:bg-indigo-900/30', labelKey: 'notifTypeAnnouncement' },
  new_exam: { icon: FiCalendar, color: 'text-blue-500', bg: 'bg-blue-100 dark:bg-blue-900/30', labelKey: 'notifTypeNewExam' },
  update: { icon: FiInfo, color: 'text-yellow-500', bg: 'bg-yellow-100 dark:bg-yellow-900/30', labelKey: 'notifTypeUpdate' },
  reminder: { icon: FiBell, color: 'text-orange-500', bg: 'bg-orange-100 dark:bg-orange-900/30', labelKey: 'notifTypeReminder' },
  general: { icon: FiBell, color: 'text-gray-500', bg: 'bg-gray-100 dark:bg-gray-700', labelKey: 'notifTypeGeneral' },
};
...
const NotificationBell = () => {
  ...
  const getTypeConfig = (type) => typeConfigBase[type] || typeConfigBase.general;
          ...
          >
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              ...
              {unreadCount > 0 && (
                ...
                >
                  <FiCheckCircle className="w-3.5 h-3.5" /> {t('notifMarkAllRead')}
                </button>
              )}
            </div>
            ...
            <div className="max-h-[400px] overflow-y-auto">
              {notifications.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-gray-500 dark:text-gray-400 text-sm">{t('noNotificationsShort')}</p>
                </div>
              ) : (
